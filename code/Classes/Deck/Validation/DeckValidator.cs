...
public static class DeckValidator
{
	public static DeckValidationReport Validate( DeckDefinition deck, DeckFormatDefinition format, IEnumerable<IDeckFormatRule>? additionalRules = null )
	{
		...
		DeckValidationReport    report           = new DeckValidationReport { FormatCode = format.Code };
		Dictionary<string, int> sectionCounts    = new Dictionary<string, int>( StringComparer.OrdinalIgnoreCase );
		Dictionary<Guid, int>   copiesByOracleId = new Dictionary<Guid, int>();

		foreach ( DeckEntry entry in deck.Entries )
		{
			if ( entry.Quantity <= 0 )
			{
				Add( report, DeckValidationIssueCode.InvalidQuantity, $"'{entry.Card.Name}' has invalid quantity " + $"{entry.Quantity}.", entry );

				continue;
			}

			if ( !format.Sections.TryGetValue( entry.Section, out DeckSectionRule? sectionRule ) )
			{
				Add( report, DeckValidationIssueCode.UnknownSection, $"Section '{entry.Section}' is not allowed in " + $"{format.DisplayName}.", entry );

				continue;
			}

			sectionCounts.TryGetValue( entry.Section, out int currentSectionCount );
			sectionCounts[entry.Section] = checked(currentSectionCount + entry.Quantity);

			NormalizedCard? card = RuntimeCardDatabase.GetCard( entry.Card.ScryfallId );

			if ( card is null )
			{
				Add( report, DeckValidationIssueCode.CardNotFound, $"Printing for '{entry.Card.Name}' is not in the " + "server card database.", entry );

				continue;
			}

			if ( entry.Card.OracleId is Guid storedOracleId && card.Gameplay.OracleId != storedOracleId )
			{
				Add( report, DeckValidationIssueCode.PrintingIdentityMismatch, $"Stored Oracle identity for '{entry.Card.Name}' " + "does not match its printing.", entry );
			...
			{
				Add( report, DeckValidationIssueCode.MissingOracleIdentity, $"'{entry.Card.Name}' has no Oracle identity.", entry );

				continue;
			}

			ValidateCardLegality( card, oracleId, entry, format, report );

			if ( sectionRule.CountsTowardCopyLimit )
			{
				copiesByOracleId.TryGetValue( oracleId, out int currentCopies );
				copiesByOracleId[oracleId] = checked(currentCopies + entry.Quantity);
			}
		}
	...
	private static void ValidateSectionCounts( DeckFormatDefinition format, Dictionary<string, int> counts, DeckValidationReport report )
	{
		foreach ( KeyValuePair<string, DeckSectionRule> pair in format.Sections )
		{
	...
	private static void ValidateCopyCounts( DeckDefinition deck, DeckFormatDefinition format, Dictionary<Guid, int> copies, DeckValidationReport report )
	{
		foreach ( KeyValuePair<Guid, int> pair in copies )
		{
			...
			if ( format.CardCopyLimits.TryGetValue( pair.Key, out int cardLimit ) )
				limit = cardLimit;
			...
			DeckEntry? entry    = deck.Entries.Find( candidate => candidate.Card.OracleId == pair.Key );
			string     cardName = entry?.Card.Name ?? pair.Key.ToString();

			report.Issues.Add( new DeckValidationIssue { Code = format.RestrictedOracleIds.Contains( pair.Key )? DeckValidationIssueCode.CardRestricted : DeckValidationIssueCode.TooManyCopies, OracleId = pair.Key, ScryfallId = entry?.Card.ScryfallId, Message = $"'{cardName}' has {pair.Value} copies; " + $"the maximum is {limit}." } );
		}
	}


	private static void Add( DeckValidationReport report, DeckValidationIssueCode code, string message, DeckEntry entry )
	{
		report.Issues.Add(
						  new DeckValidationIssue
						  {
							  ...
							  ScryfallId = entry.Card.ScryfallId == Guid.Empty? null : entry.Card.ScryfallId,
