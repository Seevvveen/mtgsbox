...
public static class JsonArrayReader
{
	...
	/// </summary>
	public static void ReadObjectBytes(
		Stream stream,
		Action<ReadOnlyMemory<byte>, int> onObject,
		int readBufferSize = 64 * 1024 )
	{
		...
		void ProcessByte( byte current )
		{
			if ( readingObject )
			{
				...
				if ( insideString )
				{
					if ( escaped )
					{
						// The current character was escaped.
						escaped = false;
					}
				...
				if ( structureDepth < 0 )
				{
					throw new JsonException(
						$"Invalid JSON structure near array index {objectIndex}."
					);
				}
			...
			switch ( state )
			{
				...
				{
					throw new JsonException(
						"Unexpected non-whitespace data after the JSON array."
					);
				}
			}
		}
		...
		int prefixStart = HasUtf8Bom( prefix, prefixLength )
			? 3
			: 0;
		...
		{
			throw new JsonException(
				$"JSON ended before the object at array index " +
				$"{objectIndex} was complete."
			);
		}
		...
		{
			throw new JsonException(
				"JSON ended before the top-level array was completed."
			);
		}
	}
	...
	/// </summary>
	public static void ReadObjects<T>(
		Stream stream,
		...
		int readBufferSize = 64 * 1024 )
	{
		...
		ReadObjectBytes(
			...
			(jsonBytes, index) =>
			{
				...
				{
					throw new JsonException(
						$"The JSON object at array index {index} " +
	...
	private static void StartObject(
		byte current,
		MemoryStream objectBuffer,
		ref bool readingObject,
		ref bool insideString,
		ref bool escaped,
		ref int structureDepth,
		int objectIndex )
	{
		if ( current != (byte)'{' )
		{
			throw new JsonException(
				$"Expected a JSON object at array index {objectIndex}."
			);
		}
