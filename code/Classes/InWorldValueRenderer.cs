...
public sealed class InWorldValueRenderer : Component
{
		...
		Vector3 localOffset )
	{
		...
		SetValue(
			...
			CardMesh.Width * 1.25f );
	}

	public void SetManaPool(
		string key,
		IReadOnlyDictionary<ManaType, int> mana,
		Vector3 localOffset )
	{
		...
		Action? callback )
	{
		...
		if ( !_entries.TryGetValue( key, out Entry? entry ) )
		{
			var child = new GameObject(
				...
				true,
				$"World Value: {key}" );
			var decal = child.Components.Create<Decal>();
			decal.Depth = MathF.Max(
				CardMesh.Thickness * 4f,
				2f );
			decal.Rotation = 0f;
		...
		entry.Decal.Size = new Vector2(
			width,
			width / InWorldValueTextureRenderer.Aspect );

		Texture texture =
			InWorldValueTextureRenderer.BuildLabel( text, style );
		...
		{
			entry.Action ??=
				entry.GameObject.Components
					.Create<InWorldActionButton>();
			entry.Action.Configure( key, callback );
			var collider =
				entry.GameObject.Components
					.GetOrCreate<BoxCollider>();
			collider.IsTrigger = true;
			collider.Scale = new Vector3(
				width,
				width / InWorldValueTextureRenderer.Aspect,
				MathF.Max( CardMesh.Thickness, 1f ) );
		}
	...
	{
		entry.GameObject.WorldPosition =
			WorldPosition + WorldRotation * entry.LocalOffset;
		entry.GameObject.WorldRotation = Rotation.LookAt(
			-WorldRotation.Up,
			WorldRotation.Left );
	}

	private static void AppendMana(
		StringBuilder text,
		IReadOnlyDictionary<ManaType, int> mana,
		ManaType type,
		string symbol )
	{
		if ( !mana.TryGetValue( type, out int amount ) ||
			amount <= 0 )
		{
			return;
		}
