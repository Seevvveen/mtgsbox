...
public static class ScryfallCardNormalizer
{
				   ...
				   Gameplay = new CardGameplayData
							  {
								  ...
								  ColorIndicator = ParseOptionalColors( dto.ColorIndicator, "color_indicator" ),
								  Defense        = ToDefense( dto.Defense ),
								  HandModifier   = ToHandModifier( dto.HandModifier ),
								  Keywords       = new CardKeywords { Values = CopyArray( dto.Keywords ) },
				   ...
				   Presentation = new CardPresentationData
								  {
									  ...
									  IllustrationId      = ParseOptionalGuid( dto.IllustrationId, "illustration_id" ),
									  Images              = NormalizeImages( dto.ImageUris ),
									  ImageStatus         = RequireString( dto.ImageStatus, "image_status" ),
									  ImageUpdatedAt      = dto.ImageUpdatedAt,
									  Oversized           = dto.Oversized,
									  Prices              = NormalizePrices( dto.Prices ?? throw MissingField( "prices" ) ),
									  ...
									  VariationOf         = ParseOptionalGuid( dto.VariationOf, "variation_of" ),
				   ...
				   Set = new CardSetData
						 {
							 ...
							 SearchUri   = RequireString( dto.SetSearchUri, "set_search_uri" ),
							 ScryfallUri = RequireString( dto.ScryfallSetUri, "scryfall_set_uri" )
						 },
				   Links = new CardResourceLinks
						   {
							   ApiUri          = RequireString( dto.Uri, "uri" ),
							   ScryfallUri     = RequireString( dto.ScryfallUri, "scryfall_uri" ),
							   PrintsSearchUri = RequireString( dto.PrintsSearchUri, "prints_search_uri" ),
							   RulingsUri      = RequireString( dto.RulingsUri, "rulings_uri" ),
							   PurchaseUris    = CopyNullableDictionary( dto.PurchaseUris ),
	...
	private static CardFace[] NormalizeFaces( ScryfallCardDto dto )
	{
		...
		for ( int index = 0; index < dto.CardFaces.Length; index++ )
		{
			ScryfallCardFaceDto source = dto.CardFaces[index] ?? throw new InvalidDataException( $"Scryfall field 'card_faces[{index}]' cannot be null." );

			string prefix = $"card_faces[{index}]";

			faces[index] = new CardFace
						   {
							   Object           = RequireObjectKind( source.Object, $"{prefix}.object", "card_face" ),
							   Name             = RequireString( source.Name, $"{prefix}.name" ),
							   SourceManaCost   = source.ManaCost,
							   ManaCost         = ParseFaceManaCost( source.ManaCost, $"{prefix}.mana_cost" ),
							   TypeLine         = source.TypeLine,
							   OracleText       = source.OracleText,
							   Colors           = ParseOptionalColors( source.Colors, $"{prefix}.colors" ),
							   ColorIndicator   = ParseOptionalColors( source.ColorIndicator, $"{prefix}.color_indicator" ),
							   ...
							   ArtistId         = ParseOptionalGuid( source.ArtistId, $"{prefix}.artist_id" ),
							   IllustrationId   = ParseOptionalGuid( source.IllustrationId, $"{prefix}.illustration_id" ),
							   ...
							   OracleId         = ParseOptionalGuid( source.OracleId, $"{prefix}.oracle_id" ),
	...
	{
		Guid[]? artistIds = ParseOptionalGuidArray( dto.ArtistIds, "artist_ids" );
		...
		return new CardFace
			   {
				   ...
				   Defense         = ToDefense( dto.Defense ),
				   Artist          = dto.Artist,
				   ArtistId        = artistIds is { Length: 1 }? artistIds[0] : null,
				   IllustrationId  = ParseOptionalGuid( dto.IllustrationId, "illustration_id" ),
				   Images          = NormalizeImages( dto.ImageUris ),
				   FlavorName      = dto.FlavorName,
				   FlavorText      = dto.FlavorText,
	...
	private static RelatedCard[]? NormalizeRelatedCards( ScryfallRelatedCardDto[]? source )
	{
		if ( source is null )
			return null;
		...
		RelatedCard[] result = new RelatedCard[source.Length];

		for ( int index = 0; index < source.Length; index++ )
		{
			ScryfallRelatedCardDto item = source[index] ?? throw new InvalidDataException( $"Scryfall field 'all_parts[{index}]' cannot be null." );
			...
			result[index] = new RelatedCard
							{
								...
								Object           = RequireString( item.Object, $"{prefix}.object" ),
								Component        = RequireString( item.Component, $"{prefix}.component" ),
								Name             = RequireString( item.Name, $"{prefix}.name" ),
								TypeLine         = RequireString( item.TypeLine, $"{prefix}.type_line" ),
								ApiUri           = RequireString( item.Uri, $"{prefix}.uri" ),
								SourceExtensions = CopyExtensions( item.AdditionalFields )
							};
		}
	...
	private static CardImages? NormalizeImages( ScryfallImageUrisDto? source )
	{
		if ( source is null )
			return null;
	...
	private static CardPrices NormalizePrices( ScryfallPricesDto source )
	{
	...
	private static CardPreview? NormalizePreview( ScryfallPreviewDto? source )
	{
		if ( source is null )
			return null;

		return new CardPreview { PreviewedAt = source.PreviewedAt, SourceUri = source.SourceUri, Source = source.Source, SourceExtensions = CopyExtensions( source.AdditionalFields ) };
	}


	private static FormatLegalities ParseLegalities( Dictionary<string, string>? values )
	{
		if ( values is null )
	...
	private static CardFinish[] ParseFinishes( string[]? values )
	{
		...
		CardFinish[] results = new CardFinish[values.Length];
	...
	private static ManaCost ParseFaceManaCost( string? value, string field )
	{
		if ( value is null )
	...
	private static ColorSet ParseRequiredColors( string[]? values, string field )
	{
		if ( values is null )
	...
	private static ColorSet ParseColors( string[] values, string field )
	{
		...
		{
			throw new InvalidDataException( $"Scryfall field '{field}' contains an invalid color.", exception );
		}
	}


	private static ProducedManaSet? ParseProducedMana( string[]? values )
	{
		try
		{
		...
		{
			throw new InvalidDataException( "Scryfall field 'produced_mana' contains an invalid value.", exception );
		}
		catch ( FormatException exception )
		{
			throw new InvalidDataException( "Scryfall field 'produced_mana' contains an invalid value.", exception );
	...
	private static FrameEffect[]? ParseFrameEffects( string[]? values )
	{
		...
		FrameEffect[] results = new FrameEffect[values.Length];
	...
	private static Guid? ParseOptionalGuid( string? value, string field )
	{
		...
		throw new InvalidDataException( $"Scryfall field '{field}' contains invalid GUID '{value}'." );
	}


	private static Guid[]? ParseOptionalGuidArray( string[]? values, string field )
	{
		if ( values is null )
			return null;
	...
	private static Dictionary<string, JsonElement> CopyExtensions( Dictionary<string, JsonElement>? values )
	{
		...
		Dictionary<string, JsonElement> result = new Dictionary<string, JsonElement>( values.Count, StringComparer.Ordinal );
	...
	private static string RequireObjectKind( string? value, string field, string expected )
	{
