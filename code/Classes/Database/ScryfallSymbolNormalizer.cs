...
{
	public static CardSymbolDefinition Normalize(
		ScryfallCardSymbolDto dto )
	{
			...
			StringComparison.Ordinal ) )
		{
			throw new InvalidDataException(
				$"Expected a Scryfall card_symbol object, but received " +
				$"'{dto.Object}'." );
		}
		...
		return new CardSymbolDefinition
		{
			...
			Colors = ColorSet.FromScryfall(
				dto.Colors
					?? throw MissingField( "colors" ) ),
			GathererAlternates = dto.GathererAlternates is null
				? null
				: [.. dto.GathererAlternates],
	...
	private static SymbolIdentifier ParseIdentifier( string value )
	{
		try
		{
			return SymbolIdentifier.Parse(
				RequireString( value, "symbol" ) );
		}
		catch ( ArgumentException exception )
		{
			throw new InvalidDataException(
				$"Scryfall field 'symbol' contains invalid identifier " +
				$"'{value}'.",
				exception );
		}
	...
	private static InvalidDataException MissingField( string field )
	{
		return new InvalidDataException(
			$"Required Scryfall symbol field '{field}' is missing." );
	}
