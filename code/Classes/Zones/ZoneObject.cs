...
public sealed class ZoneObject : Component
{
	...
	[Property] public Vector3 TriggerSize { get; set; } = new Vector3( CardMesh.DefaultWidth, CardMesh.DefaultWidth / CardFaceRenderer.Aspect, 1f );

	public IReadOnlyList<CardObject> Cards
	{
		get { return _cards; }
	}
	...
	{
		get { return _cards.Count == 0? null : _cards[^1]; }
	}

	public ZoneLayout ActiveLayout
	{
		get { return UseRecommendedLayout? RecommendedLayout( ZoneType ) : Layout; }
	}
	...
	private void RefreshProxyMembership()
	{
		...
		_cards.Sort( ( left, right ) => left.StackIndex.CompareTo( right.StackIndex ) );
	}
	...
	public bool CanAccept( CardObject card )
	{
		...
		return Capacity <= 0 || _cards.Contains( card ) || _cards.Count < Capacity;
	}
	...
	public void Shuffle()
	{
		...
		for ( int index = _cards.Count - 1; index > 0; index-- )
		{
			...
			( _cards[index], _cards[other] ) = ( _cards[other], _cards[index] );
		}
	...
	public void MoveCard( int fromIndex, int toIndex, bool animate = true )
	{
		RequireAuthority();
	...
	public void RefreshConfiguration()
	{
		...
		SlotRenderer.BuildSlot( ZoneType, MarkerResolution ).ApplyTo( _markerRenderer );
	}
	...
	public Transform GetCardPose( int index )
	{
		...
		switch ( ActiveLayout )
		{
			...
			case ZoneLayout.Fan:
			{
				float centered = index - MathF.Max( CardCount - 1, 0 ) * 0.5f;
				float angle    = centered                                                * 4f;
				position += right                              * ( centered              * ( CardMesh.Width * 0.42f + CardSpacing ) );
				position -= forward                            * ( MathF.Abs( centered ) * 0.12f );
				position += normal                             * ( index                 * StackSpacing );
				rotation =  Rotation.FromAxis( normal, angle ) * rotation;
	...
	private float EffectiveStackSpacing()
	{
		...
		if ( ZoneType is ZoneType.Library or ZoneType.Graveyard or ZoneType.Exile )
		{
	...
	private bool RemoveCard( CardObject card, bool clearMembership, bool reflow )
	{
		RequireAuthority();
	...
	{
		MtgZoneCardState state = requested == MtgZoneCardState.ZoneDefault? DefaultCardState( ZoneType ) : requested;
	...
	private static MtgZoneCardState DefaultCardState( ZoneType zone )
	{
		return zone switch
			   {
				   ZoneType.Library                                                           => MtgZoneCardState.Concealed,
				   ZoneType.Hand or ZoneType.Sideboard                                        => MtgZoneCardState.OwnerOnly,
				   ZoneType.Battlefield                                                       => MtgZoneCardState.Front,
				   ZoneType.Graveyard or ZoneType.Exile or ZoneType.Command or ZoneType.Stack => MtgZoneCardState.Front,
				   _                                                                          => MtgZoneCardState.Preserve
			   };
	}


	private static ZoneLayout RecommendedLayout( ZoneType zone )
	{
		return zone switch
			   {
				   ZoneType.Hand        => ZoneLayout.Fan,
				   ZoneType.Battlefield => ZoneLayout.Freeform,
				   _                    => ZoneLayout.Stack
			   };
	}
