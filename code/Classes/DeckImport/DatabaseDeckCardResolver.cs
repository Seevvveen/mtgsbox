...
public sealed class DatabaseDeckCardResolver : IDeckCardResolver
{
	...
	{
		if ( query.ScryfallId is Guid scryfallId &&
		...
		if ( !string.IsNullOrWhiteSpace( query.SetCode ) &&
			!string.IsNullOrWhiteSpace( query.CollectorNumber ) )
		{
			NormalizedCard? byPrinting = RuntimeCardDatabase.FindPrinting(
				NormalizeSetCode( query.SetCode ),
		...
		NormalizedCard[] matches =
			RuntimeCardDatabase.FindByName( query.Name );
		...
		return new DeckCardResolution
		{
			Card = CreateReference( matches[0] ),
	...
	private static DeckCardResolution FromSingle( NormalizedCard? card )
	{
		return new DeckCardResolution
		{
			Card = card is null ? null : CreateReference( card ),
			MatchCount = card is null ? 0 : 1
		};
	}
