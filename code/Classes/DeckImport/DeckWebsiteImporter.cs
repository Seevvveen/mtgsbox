...
public sealed class DeckWebsiteImporter
{
	...
	private static readonly string[]         SectionOrder = [ DeckSections.Commander, DeckSections.Companion, DeckSections.Main, DeckSections.Sideboard, DeckSections.Maybeboard ];
			...
			string deckText = request.Kind switch
							  {
								  ...
								  LegacyPayloadKind.DeckboxHtml => ConvertDeckboxHtmlToText( payload ),
								  _                             => payload
							  };

			return _textImporter.Import( deckText, WithSource( options, url, request.Site, GetLastPathSegment( uri ) ) );
		}
		catch ( Exception exception )
		{
			return Failure( options, url, DeckImportIssueCode.RequestFailed, $"Deck request failed: {exception.Message}" );
		}
	}


	private async Task<DeckImportResult> ImportMoxfieldAsync( Uri uri, DeckImportOptions options, CancellationToken cancellationToken )
	{
		...
		string json     = await RequestStringAsync( endpoint, cancellationToken, "https://www.moxfield.com/" );

		using JsonDocument document = JsonDocument.Parse( json );
		JsonElement        root     = document.RootElement;
		StringBuilder      text     = new StringBuilder();

		AppendMoxfieldBoard( root, "commanders", DeckSections.Commander, text );
		AppendMoxfieldBoard( root, "companions", DeckSections.Companion, text );
		AppendMoxfieldBoard( root, "mainboard", DeckSections.Main, text );
		AppendMoxfieldBoard( root, "sideboard", DeckSections.Sideboard, text );
		AppendMoxfieldBoard( root, "maybeboard", DeckSections.Maybeboard, text );

		string deckName = GetString( root, "name" ) ?? options.DeckName;

		return _textImporter.Import( text.ToString(), WithSource( options with { DeckName = deckName }, uri.ToString(), "moxfield", deckId ) );
	}


	private async Task<DeckImportResult> ImportArchidektAsync( Uri uri, DeckImportOptions options, CancellationToken cancellationToken )
	{
		...
		string json     = await RequestStringAsync( endpoint, cancellationToken );

		using JsonDocument document = JsonDocument.Parse( json );
		JsonElement        root     = document.RootElement;
		JsonElement        rows     = root;

		if ( root.ValueKind == JsonValueKind.Object && root.TryGetProperty( "cards", out JsonElement cards ) )
			rows = cards;
		...
		{
			if ( !row.TryGetProperty( "card", out JsonElement card ) || card.ValueKind != JsonValueKind.Object )
				continue;
			...
			if ( !bySection.TryGetValue( section, out StringBuilder? sectionText ) )
			{
		...
		return _textImporter.Import( text.ToString(), WithSource( options, uri.ToString(), "archidekt", deckId ) );
	}


	private static void AppendMoxfieldBoard( JsonElement root, string property, string section, StringBuilder output )
	{
		if ( !root.TryGetProperty( property, out JsonElement board ) || board.ValueKind != JsonValueKind.Object )
			return;
		...
		foreach ( JsonProperty propertyEntry in board.EnumerateObject() )
		{
			...
			if ( !entry.TryGetProperty( "card", out JsonElement card ) )
				continue;

			string? uuid     = GetString( card, "scryfall_id" );
			string  name     = GetString( card, "name" ) ?? propertyEntry.Name;
			string  identity = uuid is null? name : $"{name} {uuid}";
	...
	{
		if ( !row.TryGetProperty( "categories", out JsonElement categories ) || categories.ValueKind != JsonValueKind.Array )
	...
	{
		if ( card.TryGetProperty( "oracleCard", out JsonElement oracleCard ) )
		{
		...
		return GetString( card, "displayName" ) ?? GetString( card, "name" );
	}
	...
	private static LegacyDeckRequest? CreateLegacyRequest( Uri uri )
	{
		...
		if ( IsHost( host, "deckstats.net" ) )
		{
			...
			return new LegacyDeckRequest( AppendQuery( baseUrl, "include_comments=1&export_txt=1" ), "deckstats", LegacyPayloadKind.Text );
		}
		...
		if ( IsHost( host, "pastebin.com" ) )
		{
			...
			return new LegacyDeckRequest( $"https://pastebin.com/raw/{id}", "pastebin", LegacyPayloadKind.Text );
		}
		...
		{
			string download = Regex.Replace( url, @"/deck/", "/deck/download/", RegexOptions.IgnoreCase );
			int    fragment = download.IndexOf( '#' );

			if ( fragment >= 0 )
				download = download[..fragment];

			return new LegacyDeckRequest( download, "mtggoldfish", LegacyPayloadKind.Text );
		}

		if ( IsHost( host, "cubecobra.com" ) )
		{
			string download = Regex.Replace( url, @"cube/deck", "cube/deck/download/mtgo", RegexOptions.IgnoreCase );

			return new LegacyDeckRequest( download, "cubecobra", LegacyPayloadKind.Text );
		}
	...
	{
		Match  body = Regex.Match( html, @"<body[^>]*>(?<body>.*?)</body>", RegexOptions.IgnoreCase | RegexOptions.Singleline );
	...
	private static string ConvertCsvToDeckText( string csv )
	{
		...
		int          setIndex       = FindHeader( headers, "set", "printing", "edition" );
		int          collectorIndex = FindHeader( headers, "collector number", "collector_number", "collector" );

		if ( quantityIndex < 0 || nameIndex < 0 )
			throw new FormatException( "Deck CSV has no quantity/name columns." );

		Dictionary<string, StringBuilder> sections = new Dictionary<string, StringBuilder>( StringComparer.OrdinalIgnoreCase );
		...
		for ( int index = 1; index < lines.Length; index++ )
		{
			...
			string section  = boardIndex >= 0 && boardIndex < row.Count? NormalizeCsvSection( row[boardIndex] ) : DeckSections.Main;
			string identity = row[nameIndex].Trim();

			if ( setIndex >= 0 && setIndex < row.Count && !string.IsNullOrWhiteSpace( row[setIndex] ) )
			{
				identity += $" ({row[setIndex].Trim().ToUpperInvariant()})";

				if ( collectorIndex >= 0 && collectorIndex < row.Count && !string.IsNullOrWhiteSpace( row[collectorIndex] ) )
					identity += $" {row[collectorIndex].Trim()}";
			}

			if ( !sections.TryGetValue( section, out StringBuilder? sectionRows ) )
			{
				sectionRows = new StringBuilder();
				sections.Add( section, sectionRows );
			}

			sectionRows.AppendLine( $"{row[quantityIndex].Trim()} {identity}" );
		}
		...
		foreach ( string section in SectionOrder )
		{
			if ( sections.TryGetValue( section, out StringBuilder? rows ) )
			{
				output.AppendLine( SectionHeading( section ) );
				output.Append( rows );
			}
	...
	private static async Task<string> RequestStringAsync( string url, CancellationToken cancellationToken, string? referer = null )
	{
		Dictionary<string, string> headers = new Dictionary<string, string> { ["Accept"] = "application/json, text/plain, text/csv, */*" };

		if ( referer is not null )
			headers["Referer"] = referer;

		return await Http.RequestStringAsync( url, "GET", null, headers, cancellationToken );
	}
	...
	private sealed record LegacyDeckRequest( string Url, string Site, LegacyPayloadKind Kind );
