...
public sealed class CardObject : Component
{
	...
	/// </summary>
	[Sync]
	public Guid RevealedPrintingId { get; set; }
	...
	public Guid KnownPrintingId
	{
		get
		{
			...
			{
				return _privatePrintingId != Guid.Empty
					? _privatePrintingId
					: _authoritativePrintingId;
			}
	...
	public bool IsConcealed => FaceIndex < 0;
	public bool HasPrintedBack { get; private set; }
	public Transform RestPose => new( _targetPosition, _targetRotation );
	...
	public void SetCard( Guid printingId )
	{
		if ( printingId == Guid.Empty )
			throw new ArgumentException(
				"Printing ID cannot be empty.",
				nameof(printingId) );

		if ( GameObject.Network.IsProxy )
		{
			throw new InvalidOperationException(
				"Only the card authority can assign its hidden identity." );
		}
	...
	public void Reveal( int faceIndex = 0 )
	{
		...
		{
			throw new InvalidOperationException(
				"Only the card authority can reveal it." );
		}

		if ( _authoritativePrintingId == Guid.Empty )
		{
			throw new InvalidOperationException(
				"Assign a printing before revealing the card." );
		}
	...
	private static void RequirePrintedBack( Guid printingId )
	{
		...
		{
			throw new InvalidOperationException(
				"The card database must be ready before selecting a " +
				"printed back face.",
				exception );
		}

		if ( card is null ||
			!CardFaceRenderer.HasPrintedBack( card ) )
		{
			throw new InvalidOperationException(
				"This printing has no second printed face." );
		}
	}
	...
	public void Conceal()
	{
		...
		{
			throw new InvalidOperationException(
				"Only the card authority can conceal it." );
		}
	...
	{
		if ( GameObject.Network.IsProxy ||
			_authoritativePrintingId == Guid.Empty )
		{
			return;
		}
	...
	protected override void OnUpdate()
	{
		...
		{
			float interpolation =
				1f - MathF.Exp( -MoveSpeed * Time.Delta );
			_easedPosition = Vector3.Lerp(
				_easedPosition,
				_targetPosition,
				interpolation );

			if ( Vector3.DistanceBetween(
				_easedPosition,
				_targetPosition ) < 0.05f )
			{
		...
		_flip = _flip.Approach(
			flipTarget,
			FlipSpeed * Time.Delta );
		_hover = _hover.Approach(
			_hoverTarget,
			Time.Delta * 10f );

		float lift =
			MathF.Sin( _flip.DegreeToRadian() ) * FlipLiftMax +
			PulseAmount() * PulseRise +
			_hover * HoverLift;
	...
	private void RefreshVisualIfChanged()
	{
		...
		bool concealed = FaceIndex < 0 ||
			RevealedPrintingId == Guid.Empty;
		string desiredKey = concealed
			? "concealed"
			: RevealedPrintingId.ToString( "N" );

		if ( _renderedConcealed == concealed &&
			(concealed ||
				_renderedPrintingId == RevealedPrintingId) )
		{
			return;
		}
	...
	private void BeginVisualRefresh()
	{
		...
		if ( card is null )
		{
			if ( _failedPrintingId != printingId )
			{
				...
				Log.Warning(
					$"CardObject could not resolve printing " +
					$"'{printingId}'." );
			}
	...
	private async Task ApplyConcealedAsync( int generation )
	{
		try
		{
			CardTextures textures =
				await CardFaceRenderer.BuildConcealedAsync();
		...
		catch ( Exception exception )
		{
			if ( generation == _renderGeneration )
			{
				_requestedVisualKey = null;
				Log.Warning(
					$"Unable to render a concealed card: " +
					$"{exception.Message}" );
			}
		}
	}
		...
		int generation )
	{
		...
		{
			CardTextures textures =
				await CardFaceRenderer.BuildCardAsync( card );
		...
		catch ( Exception exception )
		{
			if ( generation == _renderGeneration )
			{
				_requestedVisualKey = null;
				Log.Warning(
					$"Unable to render card printing " +
	...
	private void UpdateThrow()
	{
		...
		bool atRest = rigidbody is null ||
			rigidbody.Velocity.Length < ThrowRestSpeed &&
			rigidbody.AngularVelocity.Length < ThrowRestSpeed;
		_restAge = atRest ? _restAge + Time.Delta : 0f;

		if ( _restAge < ThrowRestTime &&
			_throwAge < ThrowMaxTime )
		{
			return;
		}
	...
	private void UpdateEmphasis()
	{
		...
		float glow = _glow
			? 0.3f + 0.2f *
				(0.5f + 0.5f * MathF.Sin( Time.Now * 4f ))
			: 0f;
		float amount = MathF.Max(
			MathF.Max( pulse, glow ),
			_highlightAmount );
		Color tint = _highlightAmount > 0f
			? _highlightTint
			: new Color( 1f, 0.78f, 0.25f );
		_renderer.Tint = Color.Lerp(
			Color.White,
			tint,
			amount );
	}
	...
	private float PulseAmount()
	{
		return _pulseAge < 0f
			? 0f
			: MathF.Sin(
				_pulseAge / PulseDuration * MathF.PI );
	}
