...
public static class CardFaceRenderer
{
	...
	private static readonly Dictionary<string, Task<CardTextures>> Cache     = new Dictionary<string, Task<CardTextures>>( StringComparer.Ordinal );

	/// <summary>
	///     Normalized card records omit the shared back of ordinary Magic cards.
	/// </summary>
	public static string StandardBackImageUrl { get; set; } = "https://cards.scryfall.io/back.png";
	...
	private static async Task<CardTextures> BuildConcealedCoreAsync()
	{
		...
		{
			Log.Warning( $"Unable to load the standard Magic card back from " + $"'{StandardBackImageUrl}': {exception.Message}" );
			back = RenderFallbackBack();
		}

		Texture  atlas    = await CreateAtlasAsync( back, back );
		Material material = CardMaterialFactory.Create( "mtgsbox_card_concealed", atlas );
	...
	private static async Task<Texture> LoadFrontAsync( string? url, string cardName )
	{
		if ( !string.IsNullOrWhiteSpace( url ) )
		{
			...
			{
				Log.Warning( $"Unable to load the front image for '{cardName}' " + $"from '{url}': {exception.Message}" );
			}
		}
		else
	...
	private static async Task<(Texture Back, bool HasPrintedBack)> LoadBackAsync( string? printedBackUrl, string cardName )
	{
		if ( !string.IsNullOrWhiteSpace( printedBackUrl ) )
		{
			try
			{
				return ( await LoadTextureAsync( printedBackUrl ), true );
			}
			catch ( Exception exception )
			{
				Log.Warning( $"Unable to load the printed back face for " + $"'{cardName}' from '{printedBackUrl}': " + $"{exception.Message}" );
			}
		}

		try
		{
			return ( await LoadTextureAsync( StandardBackImageUrl ), false );
		}
		catch ( Exception exception )
		{
			Log.Warning( $"Unable to load the standard Magic card back from " + $"'{StandardBackImageUrl}': {exception.Message}" );
	...
	private static async Task<Texture> CreateAtlasAsync( Texture front, Texture back )
	{
		...
		int    height      = Math.Max( Math.Max( front.Height, back.Height ), 256 );
		int    width       = Math.Max( (int)MathF.Round( height * Aspect ), 1 );

		Bitmap atlas = new Bitmap( width * 2, height );
		atlas.Clear( Color.Black );
		atlas.DrawBitmap( frontBitmap, new Rect( 0, 0, width, height ) );
		atlas.DrawBitmap( backBitmap, new Rect( width, 0, width, height ) );
	...
	{
		TaskCompletionSource<Bitmap> completion = new TaskCompletionSource<Bitmap>( TaskCreationOptions.RunContinuationsAsynchronously );

		texture.GetBitmapAsync( bitmap => completion.TrySetResult( bitmap ) );
	...
	private static Texture RenderMissingFront()
	{
		...
		bitmap.DrawRoundRect( new Rect( 24, 24, width - 48, height - 48 ), new Margin( 36 ) );
	...
	private static Texture RenderFallbackBack()
	{
		...
		bitmap.DrawRoundRect( new Rect( 42, 42, width - 84, height - 84 ), new Margin( 48 ) );
		bitmap.SetFill( new Color( 0.5f, 0.22f, 0.08f ) );
		bitmap.DrawCircle( new Vector2( width / 2f, height / 2f ), width * 0.27f );
