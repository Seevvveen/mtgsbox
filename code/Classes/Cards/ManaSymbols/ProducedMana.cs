...
{
	private static readonly ReadOnlyCollection<SymbolIdentifier> EmptySymbolsView = Array.AsReadOnly( Array.Empty<SymbolIdentifier>() );
	...
	{
		get { return _symbols ?? Array.Empty<SymbolIdentifier>(); }
	}
	...
	{
		get { return _symbolsView ?? EmptySymbolsView; }
	}

	public static ProducedManaSet Empty { get; } = new ProducedManaSet( Array.Empty<SymbolIdentifier>() );

	// Standard conveniences. These do not restrict the set to these symbols.
	public static ProducedManaSet White { get; } = FromScryfallSymbol( "W" );

	public static ProducedManaSet Blue { get; } = FromScryfallSymbol( "U" );

	public static ProducedManaSet Black { get; } = FromScryfallSymbol( "B" );

	public static ProducedManaSet Red { get; } = FromScryfallSymbol( "R" );

	public static ProducedManaSet Green { get; } = FromScryfallSymbol( "G" );

	public static ProducedManaSet Colorless { get; } = FromScryfallSymbol( "C" );
	...
	public static ProducedManaSet FromSymbols( IEnumerable<SymbolIdentifier> symbols )
	{
		ArgumentNullException.ThrowIfNull( symbols );

		List<SymbolIdentifier> collected = new List<SymbolIdentifier>();
	...
	/// </summary>
	public static ProducedManaSet FromScryfall( string[] values )
	{
		ArgumentNullException.ThrowIfNull( values );

		List<SymbolIdentifier> symbols = new List<SymbolIdentifier>( values.Length );
	...
	{
		foreach ( SymbolIdentifier existing in Values )
		{
	...
	{
		foreach ( SymbolIdentifier symbol in other.Values )
		{
			if ( !Contains( symbol ) )
				return false;
		}
	...
	public ProducedManaSet Add( SymbolIdentifier symbol )
	{
		...
		SymbolIdentifier[] result = new SymbolIdentifier[Count + 1];
	...
	public ProducedManaSet Remove( SymbolIdentifier symbol )
	{
		...
		List<SymbolIdentifier> result = new List<SymbolIdentifier>( Count - 1 );
	...
	{
		List<SymbolIdentifier> result = new List<SymbolIdentifier>( Count + other.Count );
	...
	{
		List<SymbolIdentifier> result = new List<SymbolIdentifier>();

		foreach ( SymbolIdentifier symbol in Values )
		{
	...
	{
		SymbolIdentifier[] result = new SymbolIdentifier[Count];
	...
	private static bool Contains( List<SymbolIdentifier> symbols, SymbolIdentifier candidate )
	{
		foreach ( SymbolIdentifier symbol in symbols )
		{
	...
	private static int CompareSymbols( SymbolIdentifier left, SymbolIdentifier right )
	{
		string leftText  = left.ToString();
		string rightText = right.ToString();

		int leftOrder  = GetStandardOrder( leftText );
		int rightOrder = GetStandardOrder( rightText );
