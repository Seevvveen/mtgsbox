...
/// {C}     = colorless/empty
	...
	private static ManaSymbol ParseCode( string value )
	{
		...
		if ( IsAsciiNumber( code ) )
		{
				...
				out var amount ) )
			{
				throw new FormatException(
					$"Generic mana value '{code}' is too large." );
			}

			return new ManaSymbol(
				amount.ToString( CultureInfo.InvariantCulture ),
				ManaSymbolKind.Generic,
				ColorSet.Colorless,
				amount );
		}
		...
		{
			case "W":
				return Colored( code, MagicColor.White );

			case "U":
				return Colored( code, MagicColor.Blue );

			case "B":
				return Colored( code, MagicColor.Black );

			case "R":
				return Colored( code, MagicColor.Red );

			case "G":
				return Colored( code, MagicColor.Green );

			case "C":
				return new ManaSymbol(
					code,
					ManaSymbolKind.Colorless,
					ColorSet.Colorless );
		...
		throw new FormatException(
			$"Unsupported mana symbol '{{{code}}}'." );
	}

	private static ManaSymbol ParseTwoPartSymbol(
		string code,
		string first,
		string second )
	{
		// {W/P}, {U/P}, {C/P}, etc.
		if ( second == "P" &&
			 TryGetManaComponentColors( first, out var phyrexianColors ) )
		{
			return new ManaSymbol(
				code,
		...
		// {2/W}, {2/U}, etc.
		if ( first == "2" &&
			 TryGetColor( second, out var genericHybridColor ) )
		{
			return new ManaSymbol(
				code,
				ManaSymbolKind.GenericHybrid,
				ColorSet.FromColor( genericHybridColor ) );
		}

		// {C/W}, {C/U}, etc.
		if ( first == "C" &&
			 TryGetColor( second, out var colorlessHybridColor ) )
		{
			return new ManaSymbol(
				code,
				ManaSymbolKind.ColorlessHybrid,
				ColorSet.FromColor( colorlessHybridColor ) );
		}

		// {W/U}, {B/R}, etc.
		if ( TryGetColor( first, out var firstColor ) &&
			 TryGetColor( second, out var secondColor ) &&
			 firstColor != secondColor )
		{
			return new ManaSymbol(
				code,
				ManaSymbolKind.Hybrid,
				ColorSet.FromColors( firstColor, secondColor ) );
		}
	...
	private static ManaSymbol ParseThreePartSymbol(
		string code,
		string first,
		string second,
		string third )
	{
		// {W/U/P}, {B/G/P}, etc.
		if ( third == "P" &&
			 TryGetColor( first, out var firstColor ) &&
			 TryGetColor( second, out var secondColor ) &&
			 firstColor != secondColor )
		{
			return new ManaSymbol(
				code,
				ManaSymbolKind.HybridPhyrexian,
				ColorSet.FromColors( firstColor, secondColor ) );
		}
	...
	private static ManaSymbol Colored(
		string code,
		MagicColor color )
	{
		return new ManaSymbol(
			...
			ColorSet.FromColor( color ) );
	}

	private static bool TryGetManaComponentColors(
		string value,
		out ColorSet colors )
	{
		if ( value == "C" )
