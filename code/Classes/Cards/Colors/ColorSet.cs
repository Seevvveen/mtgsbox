...
{
	private const byte ValidMask = 1 << 0 | 1 << 1 | 1 << 2 | 1 << 3 | 1 << 4;
	...
	private static readonly (MagicColor Color, byte Mask, char Symbol)[] Entries = { ( MagicColor.White, 1 << 0, 'W' ), ( MagicColor.Blue, 1 << 1, 'U' ), ( MagicColor.Black, 1 << 2, 'B' ), ( MagicColor.Red, 1 << 3, 'R' ), ( MagicColor.Green, 1 << 4, 'G' ) };
	...
	public static ColorSet Colorless { get; } = new ColorSet( 0 );
	public static ColorSet White     { get; } = new ColorSet( GetMask( MagicColor.White ) );
	public static ColorSet Blue      { get; } = new ColorSet( GetMask( MagicColor.Blue ) );
	public static ColorSet Black     { get; } = new ColorSet( GetMask( MagicColor.Black ) );
	public static ColorSet Red       { get; } = new ColorSet( GetMask( MagicColor.Red ) );
	public static ColorSet Green     { get; } = new ColorSet( GetMask( MagicColor.Green ) );
	...
	public int Count
	{
		get { return BitOperations.PopCount( _mask ); }
	}
	...
	{
		List<MagicColor> result = new List<MagicColor>( Count );

		foreach ( (MagicColor Color, byte Mask, char Symbol) entry in Entries )
		{
		...
		return value switch
			   {
				   ...
				   "C" => throw new ArgumentException( "'C' means colorless mana, not a Magic color. " + "Use ProducedManaSet for produced_mana.", nameof(value) ),

				   _ => throw new ArgumentException( $"Unknown Scryfall color value '{value ?? "<null>"}'.", nameof(value) )
			   };
	}
	...
	private static byte GetMask( MagicColor color )
	{
		...
		throw new ArgumentOutOfRangeException( nameof(color), color, "Unknown Magic color." );
	}
}
