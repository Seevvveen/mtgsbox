...
{
	[SkipHotload]
	private static readonly Dictionary<string, ZoneSlotTextures> Cache =
		new( StringComparer.Ordinal );

	private const TextFlag Centered =
		TextFlag.Center | TextFlag.DontClip;

	public static ZoneSlotTextures BuildSlot(
		MtgZoneKind zone,
		int resolution = 512 )
	{
		...
		if ( Cache.TryGetValue( key, out ZoneSlotTextures cached ) &&
			cached.Atlas.IsLoaded )
		{
			return cached;
		}
		...
		int width = Math.Max(
			(int)MathF.Round( height * CardFaceRenderer.Aspect ),
			1 );
		var face = new Bitmap( width, height );
	...
	private static void DrawZoneFace(
		Bitmap bitmap,
		MtgZoneKind zone,
		int width,
		int height )
	...
	private static string ZoneLabel( MtgZoneKind zone )
	{
		return zone switch
		{
			MtgZoneKind.Library => "LIBRARY",
			MtgZoneKind.Hand => "HAND",
			MtgZoneKind.Battlefield => "BATTLEFIELD",
			MtgZoneKind.Graveyard => "GRAVEYARD",
			MtgZoneKind.Exile => "EXILE",
			MtgZoneKind.Command => "COMMAND",
			MtgZoneKind.Stack => "STACK",
			MtgZoneKind.Sideboard => "SIDEBOARD",
			_ => "ZONE"
		};
	}

	private static Color ZoneColor( MtgZoneKind zone )
	{
		return zone switch
		{
			MtgZoneKind.Library =>
			...
			MtgZoneKind.Hand =>
				new Color( 0.36f, 0.68f, 0.92f ),
			MtgZoneKind.Battlefield =>
				new Color( 0.35f, 0.72f, 0.42f ),
			MtgZoneKind.Graveyard =>
				new Color( 0.58f, 0.48f, 0.68f ),
			MtgZoneKind.Exile =>
				new Color( 0.88f, 0.78f, 0.38f ),
			MtgZoneKind.Command =>
				new Color( 0.9f, 0.5f, 0.22f ),
			MtgZoneKind.Stack =>
				new Color( 0.7f, 0.42f, 0.88f ),
			MtgZoneKind.Sideboard =>
