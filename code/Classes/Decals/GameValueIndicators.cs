...
public sealed class ZoneCountIndicator : Component
{
	...
	private ZoneType              _lastType;
	private InWorldValueRenderer? _values;

	private           ZoneObject? _zone;
	[Property] public bool        ShowZero { get; set; } = true;

	[Property] public Color Accent { get; set; } = new Color( 0.42f, 0.72f, 1f );
...
public sealed class CardValueIndicators : Component
{
	...
	private           InWorldValueRenderer? _values;
	[Property] public bool                  ShowFlipAction { get; set; } = true;
	...
	protected override void OnUpdate()
	{
		...
		bool shouldShow = ShowFlipAction && _card.HasPrintedBack && !_card.IsConcealed;
	...
	public void SetQuickAction( string key, string label, Action<CardObject> callback, Color accent, int row = 0 )
	{
		...
		_values?.SetAction(
						   $"action:{key}",
						   label,
						   new Vector3( CardMesh.Width * 0.62f, -row * CardMesh.Height * 0.13f, CardMesh.Thickness ),
						   accent,
						   () => callback( card ),
						   CardMesh.Width * 0.5f
						  );
