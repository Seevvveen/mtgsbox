...
public sealed record DeckFormatDefinition
{
	...
	public Dictionary<string, DeckSectionRule> Sections { get; init; } =
	...
	public static DeckFormatDefinition Constructed(
		string code,
		string displayName,
		int minimumMainDeckSize = 60,
		int maximumSideboardSize = 15 )
	{
		return new DeckFormatDefinition
		{
			Code = code,
			DisplayName = displayName,
			Sections = new Dictionary<string, DeckSectionRule>(
				StringComparer.OrdinalIgnoreCase )
			{
				[DeckSections.Main] = new()
				{
					MinimumCards = minimumMainDeckSize,
					CountsTowardCopyLimit = true
				},
				[DeckSections.Sideboard] = new()
				{
					MaximumCards = maximumSideboardSize,
					CountsTowardCopyLimit = true
				}
			}
		};
